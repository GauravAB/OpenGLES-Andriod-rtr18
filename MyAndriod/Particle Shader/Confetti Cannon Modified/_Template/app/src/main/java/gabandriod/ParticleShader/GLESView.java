package com.gabandroid.particleSystemModified;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer,OnGestureListener,OnDoubleTapListener
{
	...
	private void createPoints(int width , int height)
	{
		...
		for(i = 0.5f / width - 0.5f; i < 0.5f ; i = i + 1.0f / width )
		{
			...
			{
				vertsBuffer.put(0.5f*(float)Math.cos(theta));
				vertsBuffer.put(0.5f*(float)Math.sin(theta));
				vertsBuffer.put(j*50.0f);

				colorsBuffer.put( (rand.nextFloat()) * 0.5f + 0.5f);
				colorsBuffer.put( (rand.nextFloat()) * 0.5f + 0.5f);
				colorsBuffer.put( (rand.nextFloat()) * 0.5f + 0.5f);
			
				velocitiesBuffer.put((float)Math.cos(theta));
				velocitiesBuffer.put((float)Math.sin(theta));
				velocitiesBuffer.put( (rand.nextFloat()) +10.0f * 10.0f);
			
				startTimeBuffer.put( (rand.nextFloat()) + 1.4f * 10.0f);
