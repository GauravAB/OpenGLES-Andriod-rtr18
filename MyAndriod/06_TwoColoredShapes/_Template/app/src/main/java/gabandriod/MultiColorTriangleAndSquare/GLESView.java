package com.gabandroid.two_multicolor_objects;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer,OnGestureListener,OnDoubleTapListener
{
	...
	private void initialize(GL10 gl)
	{
		...
		GLES31.glBindAttribLocation(shaderProgramObject,GLESMacros.GAB_ATTRIBUTE_COLOR,"vColor");
		...
		FloatBuffer verticesBufferTriangle = byteBuffer.asFloatBuffer();
		//data inside buffer
		verticesBufferTriangle.put(triangleVertices);
		//start from
		verticesBufferTriangle.position(0);
		GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER,triangleVertices.length*4,verticesBufferTriangle,GLES31.GL_STATIC_DRAW);
		...
		GLES31.glGenBuffers(1,vbo_triangle_color,0);
		GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER,vbo_triangle_color[0]);

		byteBuffer = ByteBuffer.allocateDirect(triangleColors.length*4);
		byteBuffer.order(ByteOrder.nativeOrder());
		FloatBuffer colorBufferTriangle = byteBuffer.asFloatBuffer();
		colorBufferTriangle.put(triangleColors);
		colorBufferTriangle.position(0);

		GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER,triangleColors.length*4,colorBufferTriangle,GLES31.GL_STATIC_DRAW);
		...
		GLES31.glVertexAttrib3f(GLESMacros.GAB_ATTRIBUTE_COLOR,1.0f,1.0f,0.0f);
		GLES31.glEnableVertexAttribArray(GAB_ATTRIBUTE_COLOR);
		GLES31.glBindVertexArray(0);
		//---------------------------VAO SQAURE END------------------------------------------------------
	...
	private void display()
	{
		...
		Matrix.translateM(modelViewMatrix,0,-1.3f,0.0f,-4.0f);
	...
	private void uninitialize()
	{
		...
		{
			GLES31.glDeleteBuffers(1,vbo_triangle_pos,0);
			vbo_triangle_pos[0] = 0;
		}

		if(vbo_triangle_color[0] != 0)
		{
			GLES31.glDeleteBuffers(1,vbo_triangle_color,0);
