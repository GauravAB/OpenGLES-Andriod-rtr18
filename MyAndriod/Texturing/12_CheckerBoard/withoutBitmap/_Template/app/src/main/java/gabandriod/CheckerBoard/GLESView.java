package com.gabandroid.checkerBoardwithoutBitmap;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer,OnGestureListener,OnDoubleTapListener
{
	...
	private final int checker_image_width = 64;
	private final int checker_image_height = 64;
	private final int[][][] check_image = new int[checker_image_height][checker_image_width][4];
	...
	private void initialize(GL10 gl)
	{
		...
		//LOAD TEXTURE----------------------------------------------------------------------------------------
		texture_checker_board[0] = loadGLTexture();
		//----------------------------------------------------------------------------------------------------
		...
		//data inside buffer
		verticesBufferSquare.put(0);
		//start from
		verticesBufferSquare.position(0);

		GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER,12*4,verticesBufferSquare,GLES31.GL_DYNAMIC_DRAW);
	...
	private int loadGLTexture()
	{
		...
		for (i=0 ; i <checker_image_height; i++)
		{
			for(j=0 ; j < checker_image_width ; j++)
				{
					c = (((i & 0x8)==0) ^ ((j & 0x8) ==0)) ? 1 * 255 : 0 * 255;
					check_image[i][j][0] = (byte)c;
					check_image[i][j][1] = (byte)c;
					check_image[i][j][2] = (byte)c;
					check_image[i][j][3] = (byte)255;
				}
		}

		
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(64*64*4*4);
		//set order of bytes (endianness and all)
		byteBuffer.order(ByteOrder.nativeOrder());
		//float type of buffer
		//CharBuffer textureBufferSquare = byteBuffer.asCharBuffer();
		//data inside buffer

		for(i=0 ; i < checker_image_width ; i++)
		{
			for(j=0 ; j < checker_image_height ; j++)
			{
				for(k=0 ; k < 4 ; k++)
				{
					byteBuffer.put((byte)check_image[i][j][k]);
				}
			}
		}
		...
		//Bitmap mybitmap = Bitmap.createBitmap(64,64,Bitmap.Config.ARGB_8888);

		//mybitmap.copyPixelsFromBuffer(byteBuffer);
		...
		//GLUtils.texImage2D(GLES31.GL_TEXTURE_2D,0,GLES31.GL_RGBA,mybitmap,0);
		
		 GLES31.glTexImage2D(GLES31.GL_TEXTURE_2D,0,GLES31.GL_RGBA,checker_image_width,checker_image_height,0,GLES31.GL_RGBA,GLES31.GL_UNSIGNED_BYTE,byteBuffer);
	...
	private void resize(int width , int height)
	{
		...
		{
			Matrix.perspectiveM(perspectiveProjectionMatrix,0,60.0f,(float)height/(float)width,0.1f,30.0f);
		}
	}
	...
	private void display()
	{
		...
		GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER,vbo_square_pos[0]);
		GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER,squareVertices.length * 4,verticesBufferSquare,GLES31.GL_DYNAMIC_DRAW);
		GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER,0);

		//bind vao
		//activate and bind texture
		GLES31.glActiveTexture(GLES31.GL_TEXTURE0);
		GLES31.glBindTexture(GLES31.GL_TEXTURE_2D,texture_checker_board[0]);
		...
		float squareVerticesNew[] = new float[] 		
		{
		...
		//create c like buffer
		byteBuffer = ByteBuffer.allocateDirect(squareVerticesNew.length*4);
		//set order of bytes (endianness and all)
		byteBuffer.order(ByteOrder.nativeOrder());
		//float type of buffer
		verticesBufferSquare = byteBuffer.asFloatBuffer();
		//data inside buffer
		verticesBufferSquare.put(squareVerticesNew);
		//start from
		verticesBufferSquare.position(0);
		
		GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER,vbo_square_pos[0]);
		GLES31.glBufferData(GLES31.GL_ARRAY_BUFFER,squareVerticesNew.length * 4,verticesBufferSquare,GLES31.GL_DYNAMIC_DRAW);
		GLES31.glBindBuffer(GLES31.GL_ARRAY_BUFFER,0);
